...
const transformSeedToMushroom = (seed) => {
  ...
  mushroomModel.traverse((child) => {
    if (child.isMesh && child.material) {
      ...
      // Add advanced psychedelic shader for more intense effects on caps
      if (isCapPart) {
        // Create an advanced shader material for cap parts
        const shaderMaterial = new THREE.ShaderMaterial({
          ...
          fragmentShader: `
            ...
            void main() {
              // Create complex, flowing psychedelic patterns
              
              // Base coordinate transformations
              vec2 uv = vUv;
              float nx = vPosition.x * 2.0;
              float ny = vPosition.y * 2.0;
              float nz = vPosition.z * 2.0;
              
              // Create flowing noise patterns with multiple layers
              float noiseScale1 = 3.0;
              float timeScale1 = time * 0.1;
              float noise1 = snoise(vec3(nx * noiseScale1, ny * noiseScale1, timeScale1)) * 0.5 + 0.5;
              
              float noiseScale2 = 7.0;
              float timeScale2 = time * 0.15 + 10.0;
              float noise2 = snoise(vec3(nx * noiseScale2, ny * noiseScale2, timeScale2)) * 0.5 + 0.5;
              
              // Add ripple pattern
              float dist = length(vPosition.xz);
              float ripple = sin(dist * 10.0 - time * 0.5) * 0.5 + 0.5;
              
              // Combine patterns
              float pattern = mix(noise1, noise2, 0.5) * 0.7 + ripple * 0.3;
              
              // Create dynamic color shifts based on the pattern
              float hue = pattern * 0.8 + time * 0.05;
              float sat = 0.7 + pattern * 0.3; // More saturated in bright areas
              float val = 0.6 + pattern * 0.4; // Brighter in pattern highlights
              
              // Create color bands/stripes flowing across the surface
              float bands = sin(vPosition.y * 15.0 + noise1 * 5.0 + time * 0.2) * 0.5 + 0.5;
              hue = mix(hue, fract(hue + 0.33), bands * 0.7); // Shift hue in bands
              
              // Convert HSV to RGB
              vec3 color = hsv2rgb(vec3(hue, sat, val));
              
              // Add glow based on normal direction (edges glow more)
              float edgeGlow = 1.0 - max(0.0, dot(vNormal, vec3(0.0, 0.0, 1.0)));
              color += vec3(0.3, 0.5, 1.0) * edgeGlow * 0.5;
              
              // Add sparkles/stars
              float sparkleNoise = snoise(vec3(nx * 20.0, ny * 20.0, time * 0.5));
              if (sparkleNoise > 0.93) {
                color += vec3(1.0, 1.0, 1.0) * (sparkleNoise - 0.93) * 10.0;
              }
        ...
        // Replace the standard material with the shader material
        child.material = shaderMaterial;
        
        // Store shader material reference for animation updates
        child.userData.shaderMaterial = true;
      } else {
        // Set the standard material for stem parts
