    ...
    <form @submit.prevent="handleSubmit">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input
              ...
              required
            />
            <span v-if="validationErrors?.name" class="text-danger small">{{ validationErrors.name[0] }}</span>
          </div>
          ...
          <div class="col-md-6">
            ...
            <input
              ...
              required
            />
            <span v-if="validationErrors?.email" class="text-danger small">{{ validationErrors.email[0] }}</span>
          </div>
          ...
          <div class="col-md-6">
            ...
            <vue-tel-input
              ...
              :dropdown-options="{ showSearchBox: true, showDialCodeInSelection: true, showDialCodeInList: true }"
              :input-options="{ placeholder: 'Enter phone number', required: true }"
              @country-changed="handleCountryChange"
              class="vue-tel-input-custom"
            />
            <span v-if="validationErrors?.phone" class="text-danger small">{{ validationErrors.phone[0] }}</span>
          </div>
          ...
          <div class="col-md-12">
            <label for="tax_number" class="form-label">GST/VAT/Tax Number</label>
            <input
              id="tax_number"
            ...
            />
            <small class="text-muted">Provide your business tax identification number if applicable</small>
          </div>
        </div>

        <div class="step-navigation mt-4">
          <button type="button" @click="nextStep" class="btn btn-smooth-success px-4">
      ...
      <div v-show="currentStep === 1" class="form-section">
        ...
        <div class="mb-3">
          ...
          />
          <small class="d-block text-muted mt-2">Minimum {{ MIN_QUANTITY }} units per product</small>
          ...
          <div v-else-if="productSearchResults.length > 0" class="search-results-list mt-2">
            <div 
            ...
            >
              ...
              <div class="product-info-search">
                ...
                <p v-if="product.category_name" class="product-category small text-muted mb-0">{{ product.category_name }}</p>
              </div>
          ...
          <div v-else-if="productSearchQuery.length >= 2" class="search-results-list mt-2">
        ...
        <div v-if="form.products.length > 0" class="selected-products">
          ...
          <div v-for="(product, index) in form.products" :key="index" class="selected-product-card mb-3">
            <div class="product-info">
              <div class="product-details">
                <h6 class="product-name">{{ product.product_name || 'Unnamed Product' }}</h6>
                <span v-if="product.product_id" class="product-id-badge">ID: {{ product.product_id }}</span>
              </div>
            ...
            <span v-if="validationErrors?.[`products.${index}.quantity`]" class="text-danger small">
        ...
        <div v-else class="empty-products text-center py-5">
          <LucideIcon icon="mdi:package-variant-closed" size="48" class="text-muted mb-3" />
          <p class="text-muted">No products selected yet. Start typing in the search box above to add products.</p>
        </div>

        <div class="step-navigation mt-4">
          <button type="button" @click="prevStep" class="btn btn-outline-secondary px-4 me-2">
            <LucideIcon icon="mdi:arrow-left" class="me-2" /> Back
          </button>
          <button type="button" @click="nextStep" :disabled="form.products.length === 0" class="btn btn-smooth-success px-4">
            Next <LucideIcon icon="mdi:arrow-right" class="ms-2" />
          </button>
        </div>
      </div>
      ...
      <div v-show="currentStep === 2" class="form-section">
        ...
        <div class="row g-3">
          <div class="col-md-12">
            <label for="shipping_address" class="form-label">Address Line <span class="text-danger">*</span></label>
            <textarea
              id="shipping_address"
              ...
              required
            ></textarea>
            <span v-if="validationErrors?.shipping_address" class="text-danger small">
          ...
          <div class="col-md-6">
            ...
            <input
              ...
              required
            />
            <span v-if="validationErrors?.shipping_city" class="text-danger small">
              {{ validationErrors.shipping_city[0] }}
            </span>
          </div>

          <div class="col-md-6">
            <label for="shipping_state" class="form-label">State/Province <span class="text-danger">*</span></label>
            <input
              id="shipping_state"
              ...
              required
            />
            <span v-if="validationErrors?.shipping_state" class="text-danger small">
              {{ validationErrors.shipping_state[0] }}
            </span>
          </div>

          <div class="col-md-6">
            <label for="shipping_pincode" class="form-label">Pincode/Postal Code <span class="text-danger">*</span></label>
            <input
              id="shipping_pincode"
              ...
              required
            />
            <span v-if="validationErrors?.shipping_pincode" class="text-danger small">
          ...
          <div class="col-md-6">
            ...
            <input
              ...
              required
            />
            <span v-if="validationErrors?.shipping_country" class="text-danger small">
      ...
      <div v-show="currentStep === 3" class="form-section">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            <label for="budget_range" class="form-label">Budget Range (INR)</label>
            <select id="budget_range" v-model="form.budget_range" class="form-input">
        ...
        <div class="step-navigation mt-4">
          ...
          <button type="submit" :disabled="loading || !isFormValid" class="btn btn-smooth-success btn-lg px-5">
            <span v-if="loading">
...
import { useBulkOrderInquiry, type BulkOrderProduct } from '@/composables/useBulkOrderInquiry'
...
const isFormValid = computed(() => {
  return (
    ...
    form.value.products.every(p => p.product_id && p.product_name && p.quantity >= MIN_QUANTITY)
  )
})
...
onMounted(() => {
  if (userStore.user) {
    form.value.name = `${userStore.user.first_name || ''} ${userStore.user.last_name || ''}`.trim()
...
async function handleSubmit() {
  ...
  if (result.success) {
    successMessage.value = result.message || 'Your bulk order inquiry has been received!'
...
function resetForm() {
  form.value = {
    name: userStore.user ? `${userStore.user.first_name || ''} ${userStore.user.last_name || ''}`.trim() : '',
